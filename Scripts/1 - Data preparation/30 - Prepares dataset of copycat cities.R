...
stopifnot(is_unique_key(fin, c("municipality_id","account"))) #Checkpoint
stopifnot(is_unique_key(data_table = pairs_of_similar_accounts, key = c("municipality_id","i.municipality_id","account","amount"))) #Checkpoint
...
stopifnot(is_unique_key(data_table = similar_accounts, key = c("municipality_id","account","amount","copycat_id"))) #Checkpoint
...
stopifnot(is_unique_key(cities, "municipality_id")) #Checkpoint
stopifnot(length(unique(fin$municipality_id)) == nrow(cities)) #Checkpoint
stopifnot(sum(cities$n_copied_accounts) == nrow(copied_accounts)) #Checkpoint
